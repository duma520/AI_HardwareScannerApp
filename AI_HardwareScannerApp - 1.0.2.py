...
class HardwareScannerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AI加速能力检测工具 v1.0.2")
    ...
    def create_system_tab(self):
        ...
        # 使用网格布局
        for i in range(5): tab.grid_columnconfigure(i, weight=1)
    ...
    def scan_ai_support(self):
        ...
        try:
            ...
            ai_support['onnxruntime'] = {"available": False}
            self.onnx_label.config(text="未安装")
        ...
        try:
            ...
            self.tensorrt_label.config(text=f"支持 (v{tensorrt.__version__})")
        except ImportError:
            ai_support['tensorrt'] = {"available": False}
            self.tensorrt_label.config(text="未安装")
        ...
        try:
            ...
            ai_support['yolov8'] = {"available": False}
        ...
        try:
            ...
            ai_support['mobilenet'] = {"available": False}
    ...
    def write_text_report(self, file_handle):
        ...
        ai = self.results['ai_frameworks']
        file_handle.write(f"ONNX Runtime: {'是' if ai['onnxruntime']['available'] else '否'} (GPU: {'是' if ai['onnxruntime'].get('gpu', False) else '否'})\n")
        file_handle.write(f"TensorRT: {'是' if ai['tensorrt']['available'] else '否'}\n")
        file_handle.write(f"YOLOv8: {'是' if ai['yolov8']['available'] else '否'}\n")
        file_handle.write(f"MobileNet: {'是' if ai['mobilenet']['available'] else '否'}\n\n")
